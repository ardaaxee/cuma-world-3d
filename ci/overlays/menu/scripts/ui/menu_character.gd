...
func setup() -> void:
	...
	# Reuse the same production character slot and audited normalization as
	# player_controller.gd after apply_character_integration.py.
	if FileAccess.file_exists("res://assets/characters/cuma.glb"):
		...
		if resource is PackedScene:
			...
			imported_bridge.call("setup", imported)
			if bool(imported_bridge.get("available")):
				rest_y = position.y
				return
			imported.queue_free()
	...
	# This is the game's existing organic fallback, using the same Character 3.0
	# formal profile as gameplay rather than a menu-only placeholder.
	...
	if gs != null and procedural_rig.has_method("apply_hair_style"):
		procedural_rig.call("apply_hair_style", int(gs.hair_style))
	rest_y = position.y
...
func refresh_customization() -> void:
	...
	if procedural_rig.has_method("apply_palette"):
		procedural_rig.call("apply_palette", gs.get_character_palette())
