...
signal sensor_event(kind: String, position: Vector3)
...
var sensor_suspicion_until_msec = 0
var sensor_alert_until_msec = 0
var last_sensor_position = Vector3.ZERO
var last_sensor_kind = ""
...
func emit_gameplay_noise(position: Vector3, radius: float, kind: String) -> void:
	...
	for npc in _candidate_npcs():
		...
		var next_suspicion = clamp(suspicion + strength, 0.0, 1.0)
		npc.set_meta("stealth_suspicion", next_suspicion)
		npc.set_meta("stealth_last_suspicious_position", position)
		npc.set_meta("stealth_last_event", kind.left(32))
		if next_suspicion >= 0.24:
			npc.set_meta("stealth_investigate_until", now + 1800)

func report_sensor_tracking(position: Vector3, kind: String = "CCTV") -> void:
	last_sensor_position = position
	last_sensor_kind = kind.left(32)
	sensor_suspicion_until_msec = max(sensor_suspicion_until_msec, Time.get_ticks_msec() + 1400)
	sensor_event.emit("TRACKING", position)

func report_sensor_detection(position: Vector3, kind: String = "CCTV") -> void:
	last_sensor_position = position
	last_sensor_kind = kind.left(32)
	sensor_alert_until_msec = max(sensor_alert_until_msec, Time.get_ticks_msec() + 4800)
	sensor_event.emit("DETECTED", position)
	_set_alert("ALERT")
...
func get_debug_snapshot(max_agents: int = 10) -> Dictionary:
	...
	for npc in _candidate_npcs():
		...
		if player != null and npc3d.global_position.distance_to(player.global_position) > MAX_ACTIVE_DISTANCE:
			continue
		agents.append({
			"name": npc.name,
			"state": str(npc.get_meta("stealth_state", "UNAWARE")),
			"suspicion": float(npc.get_meta("stealth_suspicion", 0.0)),
			"last_position": npc.get_meta("stealth_last_suspicious_position", Vector3.ZERO),
			"last_event": str(npc.get_meta("stealth_last_event", "")),
		})
	return {
		"alert": alert_level,
		"sensor_kind": last_sensor_kind,
		"sensor_position": last_sensor_position,
		"agents": agents,
	}
...
func _candidate_npcs() -> Array:
	...
	for group_name in ["relationship_npc", "ambient_city_citizen"]:
		for npc in get_tree().get_nodes_in_group(group_name):
			if bool(npc.get_meta("stealth_perception_disabled", false)):
				continue
			if npc not in out:
				out.append(npc)
	return out
...
func _update_agent(npc: Node, delta: float) -> void:
	...
	if visible:
		...
		if suspicion >= 0.66:
			npc.set_meta("stealth_investigate_until", now + 2400)
	else:
		var decay = 0.035 if previous_state in ["INVESTIGATING", "ALERTED", "RETURNING"] else 0.075
		suspicion = max(0.0, suspicion - delta * decay)
	var investigate_until = int(npc.get_meta("stealth_investigate_until", 0))
	var new_state = "INVESTIGATING" if not visible and investigate_until > now and suspicion >= 0.14 else _state_for_suspicion(suspicion, previous_state, visible)
	npc.set_meta("stealth_suspicion", suspicion)
	npc.set_meta("stealth_state", new_state)
	if new_state == "ALERTED" and previous_state != "ALERTED":
		npc.set_meta("stealth_alert_timestamp", now)
...
func _state_for_suspicion(value: float, previous: String, visible: bool) -> String:
	if visible:
		...
		if value >= 0.14:
			return "CURIOUS"
		return "UNAWARE"
	if previous in ["INVESTIGATING", "ALERTED", "SUSPICIOUS", "RETURNING"] and value > 0.03:
		return "RETURNING"
	if value >= 0.38:
		return "SUSPICIOUS"
	if value >= 0.14:
...
func _refresh_global_alert(npcs: Array) -> void:
	...
	elif highest >= 0.14 or now < sensor_suspicion_until_msec:
