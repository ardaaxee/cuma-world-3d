...
func _build_hud() -> void:
	...
	var title = Label.new()
	title.text = "ACTION 7.3 · INTELLIGENCE"
...
func _update_status() -> void:
	...
	if observation != null and observation.has_method("get_status"):
		var recon: Dictionary = observation.call("get_status")
		if bool(recon.get("active", false)):
			var title = str(recon.get("title", ""))
			var progress = int(round(float(recon.get("progress", 0.0)) * 100.0))
			recon_text = ("ANALYZING %d%%" % progress) if not title.is_empty() else "AKTİF"
...
func _show_mission_intro() -> void:
	...
	intro_layer.name = "ActionIntro73"
...
func _open_debrief(result: Dictionary) -> void:
	...
	debrief_layer.name = "ActionDebrief73"
	...
	margin.add_child(box)
	var eyebrow = Label.new()
	eyebrow.text = "CUMA WORLD · FIELD DEBRIEF 7.3"
	eyebrow.add_theme_font_size_override("font_size", 11)
	eyebrow.add_theme_color_override("font_color", ACCENT)
	...
	var optional_completed = int(result.get("optional_completed", 0))
	var optional_total = int(result.get("optional_total", 0))
	var camera_detections = int(result.get("camera_detections", 0))
	...
	var stats = Label.new()
	stats.text = "INTEL  %d/%d   ·   OPTIONAL  %d/%d   ·   ROTA  %d\nALARM  %d   ·   CCTV  %d   ·   TEPE SEVİYE  %s\nSPYCRAFT  %d   ·   FIRSAT  %d   ·   ACTION  %d" % [intel_found, intel_total, optional_completed, optional_total, routes, alerts, camera_detections, peak, spycraft, opportunities, actions]
