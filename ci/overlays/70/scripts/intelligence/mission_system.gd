...
const MISSIONS = {
	"market_recon_70": {
		...
		"optional": ["Ek intel bul", "Alternatif girişi keşfet", "Fark edilmeden tamamla"],
		"required_intel": ["market_front_access", "market_side_access"],
		"intel": [
			{"id":"market_front_access", "title":"Ana Giriş", "description":"Yoğun ve görünür ana yaklaşım.", "category":"ACCESS_POINT", "missionId":"market_recon_70", "source":"observation", "relatedIntelIds":["market_side_access"], "optional":false, "persistence":true},
			{"id":"market_side_access", "title":"Teslimat Girişi", "description":"Daha sakin yan yaklaşım rotası.", "category":"ROUTE", "missionId":"market_recon_70", "source":"observation", "relatedIntelIds":["market_worker_route"], "optional":false, "persistence":true},
			...
			{"id":"market_dispatch_object", "title":"Teslimat Kaydı", "description":"Görevde doğrulanacak kurgu sevkiyat kaydı.", "category":"OBJECT", "missionId":"market_recon_70", "source":"mission", "relatedIntelIds":[], "optional":false, "persistence":true},
		],
		"routes": {
...
func _rank_for_result(alerts: int, found: int, total: int) -> String:
	if alerts == 0 and found >= total:
		return "GHOST"
	if alerts <= 1:
		return "SHADOW"
	return "OPERATIVE"
