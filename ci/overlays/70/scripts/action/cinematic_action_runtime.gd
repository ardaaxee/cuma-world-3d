...
func request_cover() -> bool:
	...
	if player == null:
		return false
	cover_until_msec = Time.get_ticks_msec() + 4200
	spycraft_events += 1
	_report_mission_event("COVER")
	_feedback("SİPER", "Düşük profil penceresi etkin")
	return true
...
func contextual_action() -> bool:
	...
	if player == null:
		return false
	var target = _nearest_npc(CLOSE_ACTION_DISTANCE)
	if target == null:
...
func _resolve_close_action(target: Node) -> bool:
	...
	if current == "SURRENDER":
		_feedback("AKSİYON", "Hedef zaten teslim durumda")
		return false
	target.set_meta("action_state", "STAGGERED")
	...
	_feedback("AKSİYON", "%s sendeledi" % _target_label(target))
	return true
...
func _update_recovery_states() -> void:
	...
	for npc in _candidate_npcs():
		...
		if suspension_or_civilian(npc) and suspicion >= 0.78:
			_set_npc_state(npc, "SURRENDER")
		else:
			_set_npc_state(npc, "INVESTIGATE")

func suspension_or_civilian(npc: Node) -> bool:
	return npc.is_in_group("ambient_city_citizen") or str(npc.get_meta("stealth_state", "")) == "ALERTED"
...
func _apply_npc_reactions() -> void:
	...
	for npc in _candidate_npcs():
		...
		if distance > 26.0:
			continue
		if npc.is_in_group("ambient_city_citizen") and escalation in ["SEARCH", "CHASE"]:
			_set_npc_state(npc, "FLEE")
		...
		elif escalation == "CHASE" and distance <= 4.5:
			_set_npc_state(npc, "SURRENDER")
...
func _nearest_npc(max_distance: float) -> Node:
	_resolve_player()
	if player == null:
		return null
	var best: Node = null
