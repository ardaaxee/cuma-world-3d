...
signal opportunity_marked(label: String)
...
const OPPORTUNITY_DISTANCE = 16.0
...
var opportunity_target: Node3D = null
var opportunity_label: Label3D = null
...
var action_events = 0
var opportunity_events = 0
var refresh_accum = 0.0
var chase_refresh_msec = 0
...
func get_opportunity_label() -> String:
	if opportunity_target == null or not is_instance_valid(opportunity_target):
		return "YOK"
	return str(opportunity_target.get_meta("opportunity_label", opportunity_target.name)).left(32)
...
func get_field_summary() -> Dictionary:
	...
	if action_events > 0:
		approach = "ACTION"
	elif spycraft_events > 0 or opportunity_events > 0:
		approach = "SPYCRAFT"
	return {
		"approach": approach,
...
func scan_opportunity() -> bool:
	...
	var best_score = -9999.0
	var forward = -camera.global_transform.basis.z.normalized()
	for node in get_tree().get_nodes_in_group("action_opportunity"):
		if not (node is Node3D):
			continue
		var point = node as Node3D
		var to_point = point.global_position + Vector3(0.0, 0.8, 0.0) - camera.global_position
		var distance = to_point.length()
		if distance <= 0.2 or distance > OPPORTUNITY_DISTANCE:
			continue
		var dot_value = forward.dot(to_point.normalized())
		if dot_value < 0.30:
			continue
		var score = dot_value * 2.5 - distance * 0.025
		if score > best_score:
			best_score = score
			best = point
	if best == null:
		_feedback("FIRSAT", "Yakında analiz edilebilir çevresel fırsat yok")
		return false
	_clear_opportunity()
	opportunity_target = best
	_build_opportunity_label(best)
	opportunity_events += 1
	spycraft_events += 1
	_report_mission_event("SPYCRAFT_OPPORTUNITY")
	var title = get_opportunity_label()
	var note = str(best.get_meta("opportunity_note", "Alternatif yaklaşım bilgisi"))
	opportunity_marked.emit(title)
	_feedback("FIRSAT", "%s · %s" % [title, note])
	return true
...
func _process(delta: float) -> void:
	...
	_update_mark_label()
	_update_opportunity_label()
	if Time.get_ticks_msec() >= cover_until_msec:
		active_cover_name = ""
	if escalation == "CHASE" and Time.get_ticks_msec() >= chase_refresh_msec:
		chase_refresh_msec = Time.get_ticks_msec() + 2200
		_dispatch_chase_routes()
...
func _dispatch_chase_routes() -> void:
	_resolve_player()
	if player == null:
		return
	var nodes = get_tree().get_nodes_in_group("action_chase_node")
	if nodes.is_empty():
		return
	for npc in _action_targets():
		if not (npc is Node3D) or not npc.has_method("set_ai_plan"):
			continue
		var destination = _best_chase_node((npc as Node3D).global_position, nodes)
		if destination != null:
			npc.call("set_ai_plan", "action_chase_search", (destination as Node3D).global_position, 0.26)

func _best_chase_node(origin: Vector3, nodes: Array) -> Node3D:
	var best: Node3D = null
	var best_score = INF
	for node in nodes:
		if not (node is Node3D):
			continue
		var point = node as Node3D
		var to_player = point.global_position.distance_to(player.global_position)
		var from_npc = point.global_position.distance_to(origin)
		var score = to_player * 0.70 + from_npc * 0.30
		if score < best_score:
			best_score = score
			best = point
	return best
...
func _update_npc_state_label(npc: Node, state_value: String) -> void:
	if not (npc is Node3D):
		return
	var existing = npc.get_node_or_null("ActionState72") as Label3D
...
func _build_opportunity_label(target: Node3D) -> void:
	opportunity_label = Label3D.new()
	opportunity_label.name = "QOpportunity72"
	opportunity_label.position = Vector3(0.0, 1.35, 0.0)
	opportunity_label.font_size = 28
	opportunity_label.outline_size = 8
	opportunity_label.modulate = Color(0.78, 0.92, 0.82, 0.96)
	opportunity_label.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	target.add_child(opportunity_label)
	_update_opportunity_label()
...
func _update_opportunity_label() -> void:
	if opportunity_target == null or not is_instance_valid(opportunity_target) or opportunity_label == null or not is_instance_valid(opportunity_label):
		return
	opportunity_label.text = "Q · FIRSAT\n%s" % get_opportunity_label()
...
func _player_camera() -> Camera3D:
	if player == null:
		return null
	return player.get_node_or_null("CameraPivot/CameraSpring/PlayerCamera") as Camera3D
...
func _clear_opportunity() -> void:
	if opportunity_label != null and is_instance_valid(opportunity_label):
		opportunity_label.queue_free()
...
func _feedback(kind: String, detail: String) -> void:
	...
	if p != null and p.has_method("show_status"):
		p.call("show_status", kind + " • " + detail, 1.7)
