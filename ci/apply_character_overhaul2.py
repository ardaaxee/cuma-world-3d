...
def rep(path: Path, old: str, new: str, label: str) -> None:
    ...
    if new in text and old not in text:
        print(f"CHARACTER OVERHAUL ALREADY APPLIED: {label}")
        return
    count = text.count(old)
    if count != 1:
        raise SystemExit(f"{label}: expected exactly 1 match, found {count}")
    path.write_text(text.replace(old, new, 1), encoding="utf-8")
    print(f"CHARACTER OVERHAUL APPLIED: {label}")
...
def main() -> None:
    ...
    rep(humanoid,
        ...
        "refined head silhouette")
    for old, new, label in [
        ('var shoulder_x = 0.305 * shoulder_scale','var shoulder_x = 0.292 * shoulder_scale','shoulder width'),
        ('0.058, 0.36, top, 0.91','0.050, 0.38, top, 0.80','upper arm proportions'),
        ('0.074, 0.50, pants, 0.94','0.066, 0.52, pants, 0.84','thigh proportions'),
        ('0.062, 0.46, pants.darkened(0.12), 0.94','0.055, 0.49, pants.darkened(0.08), 0.86','shin proportions'),
        ('var cadence = lerp(7.0, 11.5, run_blend)','var cadence = lerp(6.0, 9.7, run_blend)','weighted cadence'),
        ('var hip_amp = lerp(0.48, 0.78, run_blend)','var hip_amp = lerp(0.40, 0.68, run_blend)','hip swing'),
        ('var arm_amp = lerp(0.52, 0.92, run_blend)','var arm_amp = lerp(0.38, 0.72, run_blend)','arm swing'),
        ('lerp(0.018, 0.045, run_blend)','lerp(0.010, 0.027, run_blend)','root bounce'),
    ]:
    ...
    rep(player,
        ...
        "feed turn motion to rig")

    print("CUMA CHARACTER OVERHAUL 3.0: PASS")
